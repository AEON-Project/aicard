...
> **📦 Design archive**: This is the pre-launch design document for the `aicard shop` command family, kept for historical decision context (verified findings and alternative-path research included). The feature has since shipped — **for usage and integration, see the current docs**:
> [output-schema.md](./output-schema.md) (shop envelope payloads) · [exit-codes.md](./exit-codes.md) (error codes) · [recipes/integrate-in-agent.md](./recipes/integrate-in-agent.md) (programmatic orchestration) · [README](../README.md#shop--pay-on-shopify) (the 4-step flow).
> Any "pending confirmation" / milestone wording below reflects the state at design time, not the present.
